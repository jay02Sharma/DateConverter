            ...
            if (matcher.find()) {
                ...
                switch (input) {
                    ...
                    case "previous quarter":
                        ...
                        break;
                    case "next year":
                        startDate = LocalDate.of(currentDate.getYear() + 1, currentDate.getMonth(), 1);
                        endDate = startDate.plusYears(1).minusDays(1);
                        break;
                    case "next day":
                        startDate = endDate = currentDate.plusDays(1);
                        break;
                    case "next week":
                        startDate = currentDate.plusWeeks(1).with(TemporalAdjusters.nextOrSame(DayOfWeek.MONDAY));
                        endDate = startDate.plusDays(6);
                        break;
                    case "next quarter":
                        int nextQuarter = (currentDate.getMonthValue() - 1) / 3 + 1;
                        startDate = LocalDate.of(currentDate.getYear(), (nextQuarter - 1) * 3 + 1, 1);
                        endDate = startDate.plusMonths(2).with(TemporalAdjusters.lastDayOfMonth());
                        break;
                    case "next month":
                        startDate = LocalDate.of(currentDate.plusMonths(1).getYear(), currentDate.plusMonths(1).getMonth(), 1);
                        endDate = startDate.plusMonths(1).minusDays(1);
                        break;
                    default:
